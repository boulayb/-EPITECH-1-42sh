...
** Last update Sun May 17 21:58:24 2015 François CASSIN
*/
...
t_env		*create_list(char **env)
{
  ...
  if ((env_list = malloc(sizeof(t_env))) == NULL)
    {
      my_fputstr(2, "Error: Malloc failed in create_list.\n");
      return (NULL);
    }
...
int		add_chain(t_env *env_list, char *str)
{
  ...
  if (str == NULL)
    {
      my_fputstr(2, "Error: add_chain: str is NULL.\n");
      return (-1);
    }
  if ((new_chain = malloc(sizeof(t_env))) == NULL)
    {
      my_fputstr(2, "Error: Malloc failed in add_chain.\n");
      return (-1);
    }
  if ((new_chain->str = strdup(str)) == NULL)
    return (-1);
